# UW CSE312, PSet #3 Autumn 2024
# Student Name: Sameer Chakraborty
# Email    : sdc28@uw.edu
...
class NaiveBayes():
    ...
    def word_set(self, filename:str):
        ...
        Note that the data returned from this method can be assumed 
        to be pre-processed and you do not need to perfrom any 
        text pre-processing on it. 
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        for x in train_hams:
            sets = self.word_set(x)
            for y in sets:
                if y not in self.word_counts_ham:
                    self.word_counts_ham[y] = 1
                else:
                    self.word_counts_ham[y] += 1
        ...
        for x in train_spams:
            sets = self.word_set(x)
            for y in sets:
                if y not in self.word_counts_spam:
                    self.word_counts_spam[y] = 1
                else:
                    self.word_counts_spam[y] += 1
    ...
    def predict(self, filename:str):
        ...
        """
        word_set = self.word_set(filename)
    
        
        p_spam = np.log(self.num_train_spams / (self.num_train_spams + self.num_train_hams))
        p_ham = np.log(self.num_train_hams / (self.num_train_spams + self.num_train_hams))
    
        
        log_prob_spam = p_spam
        log_prob_ham = p_ham
    
       
        for word in word_set:
            # P(word | spam) with Laplace smoothing
            prob_word_spam = (self.word_counts_spam.get(word, 0) + 1) / (self.num_train_spams + 2)
            log_prob_spam += np.log(prob_word_spam)
        
            # P(word | ham) with Laplace smoothing
            prob_word_ham = (self.word_counts_ham.get(word, 0) + 1) / (self.num_train_hams + 2)
